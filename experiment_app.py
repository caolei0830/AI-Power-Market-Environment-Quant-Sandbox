...
LOCALES: dict[str, dict[str, str]] = {
    "zh": {
        ...
        "power_mw": "额定功率 (MW)",
        "capacity_mwh": "容量 (MWh)",
        "run_button": "⚡ 启动双模型打擂台（Run Experiment）",
        "mode_demo": "Demo 模拟",
    ...
    "en": {
        ...
        "run_button": "⚡ Run Dual-Model Arena (Run Experiment)",
        ...
        "guide_body": (
            ...
            "2. Open **SQL Window Architecture** to inspect ``LAG`` / ``AVG OVER`` and sample rows.  \n"
            "3. Adjust **train ratio**, then click **Run Dual-Model Arena**."
        ),
        ...
        "experiment_done": "Dual-model arena experiment completed",
        "report_board": "### Empirical Report Dashboard",
        "viz_section": "### Professional Visualizations",
...
def main() -> None:
    ...
    try:
        ...
        if use_postgres and not config.demo:
            with st.spinner(t(lang, "tier_spinner")):
                ...
                st.session_state["data_source"] = tier_label_for(lang, sourcing_ctx.tier)
                render_data_tier_banner(sourcing_ctx, lang)
    ...
    render_metric_cards(report, lang)
    render_comparison_table(report, lang)
